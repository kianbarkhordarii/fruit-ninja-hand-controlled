...
class SceneManager:
    ...
    def get_buttons(self, scene):
        """Return the list of active buttons for a given scene.
        Used by the hand cursor (dwell-to-click) so it knows what to target."""
        if scene == "MENU":
            return [self.btn_play]
        elif scene == "MODE_SEL":
            return [self.btn_classic, self.btn_survival, self.btn_back_mode]
        elif scene == "INPUT_SEL":
            return [self.btn_mouse, self.btn_hand, self.btn_back_input]
        elif scene == "PAUSE":
            return [self.btn_resume, self.btn_back_pause]
        elif scene == "OVER":
            return [self.btn_replay, self.btn_home]
        return []
