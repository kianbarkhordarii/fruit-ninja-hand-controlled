...
class HandInput(InputProvider):
    def __init__(self, width, height, shared_camera=None):
        super().__init__(width, height)
        # If a shared camera is passed in, reuse it. Otherwise open our own
        # WebcamStream.
        if shared_camera is not None:
            self.webcam = shared_camera
    ...
    def get_frame(self):
        ...
        frame = self.webcam.read() # works for both WebcamStream and SharedCamera
        if frame is not None:
             frame = cv2.flip(frame, 1)
        return frame

    def cleanup(self):
        # Never stop a camera we don't own.
        if getattr(self, "owns_camera", True):
            self.webcam.stop()
